...
def get_layers_names_by_model(model_name: str = None) -> Dict[str, str | Dict[str, str]]:
    """
    ...
    """
    path = os.path.dirname(os.path.dirname(os.path.realpath(__file__))) + "/assets/llm_models.json"
    model_dict = json.load(open(path))
    if model_name not in model_dict:
        raise ValueError(f"Model {model_name} not found in the model dictionary.")
    return model_dict[model_name] if model_name else model_dict

def identify_model(custom_model) -> str:
    """
    ...
    for model_name, layers in get_layers_names_by_model().items():
        ...
        if any(self_attention_layer in param for param in param_names) and any(mlp_layer in param for param in param_names):
